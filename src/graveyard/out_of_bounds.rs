...
impl Plugin for OutOfBoundsPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            PreUpdate,
            spawn_level_boundaries.run_if(
                any_match_filter::<Added<LevelIid>>
                    .and(in_state(GameState::Graveyard).or(in_state(GameState::LevelTransition))),
            ),
        );
    }
}
...
fn spawn_level_boundaries(
    ...
    level_iids
        .into_iter()
        .for_each(|(level_entity, level_iid)| {
